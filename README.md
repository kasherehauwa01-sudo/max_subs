...
6. После подтверждения подписки watcher отправляет купон в личный чат с ботом автоматически.
   Если MAX API долго не может проверить подписку (например, повторяющиеся `400`), срабатывает fallback-отправка купона.
...
3. при нажатии кнопки miniapp запускает watcher и открывает канал; купон отправляется после подтверждения подписки.
...
- `GET /miniapp/status?user_id=...` — служебная проверка подписки (используется backend-логикой, UI-кнопки проверки нет).
- `POST /miniapp/start-subscribe-watch` — запуск watcher, который ждёт подтверждения подписки и затем отправляет купон.
