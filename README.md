...
3. В miniapp пользователь нажимает `Проверить подписку`:
   - если подписка уже есть — активируется кнопка `Показать купон`;
   - если подписки нет — отображается кнопка `Подписаться на канал`.
4. Если подписки нет, пользователь нажимает `Подписаться на канал` и оформляет подписку в канале.
5. После этого купон приходит автоматически:
   - miniapp запускает клиентский polling;
   - backend запускает серверный watcher (`POST /miniapp/start-subscribe-watch`), который ждёт подтверждение подписки и отправляет купон в личный чат с ботом.
6. Если авто-отправка не сработала (редко), пользователь возвращается в miniapp и нажимает `Проверить подписку` → `Показать купон`.
...
6. после нажатия «Подписаться на канал» miniapp автоматически отслеживает появление подписки и отправляет купон в чат с ботом без дополнительного нажатия.
...
- `POST /miniapp/start-subscribe-watch` — запуск серверного watcher, который ждёт подписку и автоматически отправляет купон.
