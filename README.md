...
3. Отправляет ответ через MAX API методом `POST /messages`:
   ...
   - если в канале/группе/чате появляется новый подписчик (и боту приходит событие вступления), бот отправляет этому пользователю купон в личные сообщения
...
Для авто-купонов новым подписчикам убедитесь, что в webhook-подписке включены типы событий вступления/подписки (например, `chat_member_added` и аналоги, которые доступны в вашей интеграции MAX).
