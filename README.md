...
3. Отправляет ответ через MAX API методом `POST /messages`:
   - если пользователь написал `test`, `тест`, `/test`, `/hello`, `/start` или `+` → бот отправляет кнопку-ссылку для открытия миниприложения
   - если пользователь написал `КУПОН`/`coupon` → бот просит открыть miniapp через штатную кнопку `Открыть` в интерфейсе бота (без дублирующей ссылки в сообщении)
   - если пользователь написал `id`, `айди` или `/id` → бот сообщает, что отправка user_id отключена
   - если пользователь `user_id=242649311` или `user_id=24324984` написал `Дашборд` или `Статистика` → бот отправляет кнопку перехода в дашборд
   - если пользователь пришёл со стартовым payload `utm_source=qr_podpiska` и нажал `/start` → бот отправляет кнопку сценария «Подписаться на канал и получить доп. скидку -5%»

Если прямого `user_id` нет, бот пытается взять ID из альтернативных полей (`sender.id`, `user.user_id`, `profile.id` и т.д.).

### QR UTM-сценарий (`utm_source=qr_podpiska`)

- Бот отслеживает UTM в start/deep link payload.
- Для пользователей с `utm_source=qr_podpiska` команда `/start` запускает отдельный сценарий:
  1) отправка кнопки «Подписаться на канал и получить доп. скидку -5%»;
  2) по callback кнопки бот отправляет ссылку на канал и запускает отложенную отправку купона (6 секунд);
  3) при повторных нажатиях watcher может запускаться повторно.
- Для webhook события `bot_started` сценарий также срабатывает автоматически, если UTM найден в payload.

## Путь пользователя, чтобы получить купон

1. Пользователь пишет боту `Купон` (или `/coupon` / `coupon`).
2. Бот отправляет кнопку открытия miniapp.
3. В miniapp пользователь нажимает единственную кнопку `Подписаться на канал`.
4. Miniapp запускает server-side watcher (`POST /miniapp/start-subscribe-watch`) и открывает канал.
5. Пользователь подписывается на канал внутри MAX.
6. Через ~6 секунд после нажатия кнопки watcher отправляет купон в личный чат с ботом автоматически (без проверки подписки).
...
2. в интерфейсе есть только кнопка «Подписаться на канал»;
3. при нажатии кнопки miniapp запускает watcher и открывает канал; купон отправляется автоматически примерно через 6 секунд.
...
Кнопка «Подписаться на канал» в miniapp открывает `MAX_CHANNEL_DEEPLINK` (по умолчанию `https://max.ru/id344309962847_biz`) через `WebApp.openLink(...)` или `window.location.assign(...)`. Автоматического таймерного перехода в браузер нет. Web fallback (`MAX_CHANNEL_URL`) используется только если deep-link вызов выбросил ошибку.

Endpoint'ы миниприложения:
- `GET /miniapp` — HTML-интерфейс с кнопкой подписки.
- `GET /miniapp/status?user_id=...` — служебная проверка подписки.
- `POST /miniapp/start-subscribe-watch` — запуск watcher, который через ~6 секунд отправляет купон в личный чат.
...
5. В Railway добавьте переменные:
   ...
   - `GOOGLE_SCRIPT_URL=https://script.google.com/macros/s/AKfycbyU9sHM-yyJKVjYwlOfzk0whC0ba2-_xx90nKDAOGSoQG38F2UxRo4QS6sj7RufJuW2/exec`
...
- Бот допускает повторную отправку купона одному и тому же пользователю.
- В miniapp для пользователей, которые уже участвовали в акции (по данным Google Sheets), кнопка подписки может блокироваться и показываться дата участия из столбца `Дата`.

### Ежемесячное напоминание

- Каждый месяц **29 числа** бот отправляет пользователю `user_id=24324984` сообщение:
  `Обновить штрихкоды в 1с`.

### Дашборд статистики купонов

- Endpoint интерфейса: `GET /max_sub/statistic?user_id=24324984`
- Данные: `GET /max_sub/statistic/data`
- Доступ к `/dashboard` и `/dashboard/data` разрешен только для `user_id=242649311` и `user_id=24324984`.
- Периоды: `Вчера`, `Сегодня`, `Неделя`, `Месяц`, `Год`, `Ручной выбор периода`.
- Детализация диаграммы: `По часам`, `По дням`, `По неделям`, `По месяцам`.
