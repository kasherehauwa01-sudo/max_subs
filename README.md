...
3. Отправляет ответ через MAX API методом `POST /messages`:
   - если пользователь написал `test`, `тест`, `/test`, `/hello`, `/start` или `КУПОН` → бот отправляет кнопку для открытия миниприложения
...
В миниприложении:
1. пользователь вводит свой `user_id`;
2. бот проверяет подписку на канал `chat_id=208470348`;
3. если подписка есть — активна кнопка «Получить купон», и при нажатии бот отправляет купон (EAN13 + текст);
4. если подписки нет — показывается кнопка «Подписаться на канал».
...
Кнопка «Подписаться на канал» ведёт по ссылке `MAX_CHANNEL_URL` (по умолчанию `max://chat/208470348`).
...
- На Railway добавьте переменные:
  ...
  - `MAX_CHANNEL_CHAT_ID` (по умолчанию `208470348`, канал для проверки подписки)
