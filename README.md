...
- Для пользователей с `utm_source=qr_podpiska` команда `/start` запускает отдельный сценарий:
  ...
  3) действует защита от повторной выдачи купона и от повторного запуска watcher.
...
### Ограничение повторной выдачи купона

- По умолчанию бот отправляет штрихкод купона только **один раз на один `user_id`** в рамках текущего процесса приложения.
- Для `user_id=24324984` действует исключение: повторная выдача разрешена.
- В miniapp для пользователей, которые уже участвовали в акции (по данным Google Sheets), кнопка подписки блокируется и показывается дата участия из столбца `Дата`.
