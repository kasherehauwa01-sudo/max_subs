...
3. Отправляет ответ через MAX API методом `POST /messages`:
   ...
   - если пользователь написал `КУПОН` → бот отправляет купон со штрихкодом EAN13 (изображение + текст)
   - если пользователь написал `id`, `айди` или `/id` → `Ваш ID: {user_id}`
   - если в канале/группе/чате появляется новый подписчик (и приходит соответствующее событие), бот автоматически отправляет ему купон в личные сообщения
...
При команде `КУПОН` бот:
1. выбирает актуальный EAN13 по дате;
2. генерирует изображение штрихкода;
3. загружает изображение через `POST /uploads`;
4. отправляет сообщение с текстом купона и прикреплённым изображением.
...
Если платформа MAX отклоняет часть membership update types, бот автоматически подбирает поддерживаемые и регистрирует частичный набор; фактически активные типы можно посмотреть в `GET /health/config` (`active_webhook_update_types`).
