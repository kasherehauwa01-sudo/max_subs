...
2. бот проверяет подписку на канал `chat_id=72527365111160`;
...
> Кнопка «Получить купон» от бота отправляется как `open_app` (а не `link`), чтобы miniapp открывался внутри MAX с корректным WebApp-контекстом.
...
Кнопка «Подписаться на канал» ведёт по ссылке `MAX_CHANNEL_URL` (по умолчанию `max://chat/72527365111160`).
...
- На Railway добавьте переменные:
  ...
  - `MAX_WEB_APP` (опционально, явный идентификатор miniapp для кнопки `open_app`)
