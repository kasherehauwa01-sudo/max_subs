...
3. если подписка есть — активна кнопка «Показать купон», и при нажатии бот отправляет купон (EAN13 + текст);
...
Проверка подписки учитывает разные форматы ответа MAX API (вложенные `user_id/id`, списки `items/members/data`).
