...
Кнопка «Подписаться на канал» ведёт по ссылке `MAX_CHANNEL_URL` (по умолчанию `https://web.max.ru/-72559954357735`) и в miniapp открывается через `WebApp.openLink` с fallback в обычный `window.open`.
